...
class DapositionSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        cities = ['全国']
        jobs = list(set(['数据分析','数据挖掘','数据产品经理','数据分析师','数据运营','爬虫']))
        #jobs = list(set(['数据分析','数据挖掘','Python','售前工程师','产品经理','数据产品经理','数据分析师','数据运营','售前咨询','分析师','爬虫']))
        #jobs = list(set(['数据','数据分析','数据挖掘','Python','售前工程师','产品经理','数据产品经理','数据分析师','数据运营','售前咨询','商业数据分析','分析师','爬虫']))
        #jobs = list(set(['数据挖掘','Python', '售前工程师', '产品经理', '数据产品经理','数据分析师', '数据运营', '售前咨询', '分析师','爬虫']))
        ...
        for city in cities:
            for job in jobs:
                referer = 'https://www.lagou.com/jobs/list_{}?px=default&city={}'.format(urllib.parse.quote(job),
                ...
                for page in pages:
                    ...
                    #self.log('11111 ~~~~~ %s'%self.INDEX_URL_TEMPLATE.format(urllib.parse.quote(city)))
                    #self.log('22222 ~~~~~ %s'%self.parse)
                    #self.log('33333 ~~~~~ %s'%payload)
                    #self.log('44444 ~~~~~ %s'%cookies)
                    yield scrapy.http.FormRequest(
                        self.INDEX_URL_TEMPLATE.format(urllib.parse.quote(city)),
                        callback=self.parse,
                        formdata=payload,
                        cookies=cookies,
                    )
        ...
        for i in job_id:
            for j in job_results:
                ...
                if j['publisherId'] == tmp:
                    item['companyId'] = j['companyId']
                    item['positionName'] = j['positionName']
                    item['workYear'] = j['workYear']
                    item['education'] = j['education']
                    item['jobNature'] = j['jobNature']
                    item['positionId'] = j['positionId']
                    item['createTime'] = j['createTime']
                    item['city'] = j['city']
                    #item['companyLogo'] = 
                    item['industryField'] = j['industryField']
                    item['positionAdvantage'] = j['positionAdvantage']
                    item['salary'] = j['salary']
                    item['companySize'] = j['companySize']
                    item['approve'] = j['approve']
                    item['companyShortName'] = j['companyShortName']
                    if not j['positionLables']:
                        item['positionLables'] = ['暂无']
                    else:
                        item['positionLables'] = j['positionLables']
                    if not j['industryLables']:
                        item['industryLables'] = ['暂无']
                    else:
                        item['industryLables'] = j['industryLables']
                    if not j['companyLabelList']:
                        item['companyLabelList'] = ['暂无']
                    else:
                        #if j['companyLabelList'] is not types.ListType:
                        #item['companyLabelList'] = list(j['companyLabelList'])
                        item['companyLabelList'] = j['companyLabelList']
                    item['financeStage'] = j['financeStage']
                    item['district'] = j['district']
                    item['firstType'] = j['firstType']
                    item['secondType'] = j['secondType']
                    item['isSchoolJob'] = j['isSchoolJob']
                    item['companyFullName'] = j['companyFullName']

                    

                    yield item

                    '''yield scrapy.Request('http://m.lagou.com/jobs/{}.html'.format(i),
                                         meta={'data':  j},
...
''''isSchoolJob': 1,
 'city': '成都',
 'companyFullName': '四川新网银行股份有限公司',
 'companyId': 166084,
 'companyLabelList': ['带薪年假', '交通补助', '通讯津贴', '美女多'],
 'companyShortName': '新网银行',
 'companySize': '150-500人',
 'createTime': '2017-08-07 10:39:18',
 'district': '高新区',
 'education': '本科',
 ...
 'isSchoolJob': 0,
 'jobNature': '全职',
 'positionAdvantage': '互联网银行,大平台,人性化,员工食堂',
 'positionId': 3336993,
 'positionLables': ['银行', '数据分析', '数据挖掘', '大数据', '数据管理'],
 'positionName': '数据治理岗',
 'salary': '15k-25k',
 'secondType': '数据开发',
 'workYear': '3-5年'}'''  
